...
namespace RestClient.Core
{
    public class RestWebClient : Singleton<RestWebClient>
    {
        ...
        public IEnumerator HttpPostBinary(string url, string path, System.Action<Response> callback, IEnumerable<RequestHeader> headers = null)
        {
            byte[] imageData = File.ReadAllBytes(path);
            WWWForm form = new WWWForm();
            form.AddBinaryData("file", imageData, "file", "image/*");
